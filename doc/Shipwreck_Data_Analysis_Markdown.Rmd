...
```{r}
asv_otu <- read.csv("/Users/maggieshostak/Desktop/FeOB_Shipwreck_Analysis/data/asv_otu.csv")
asv_otu <- as.data.frame(t(asv_otu), headers=TRUE)
asv_otu
```
...
```{r}
otu_rel_abund <- inner_join(otu_data, taxonomy) %>%
  inner_join(., metadata, by="ASV") %>%
  group_by(ASV) %>%
